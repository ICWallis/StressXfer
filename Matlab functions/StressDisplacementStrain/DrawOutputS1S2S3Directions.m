function [S1,S2,S3,S1dir,S2dir,S3dir] = DrawOutputS1S2S3Directions(StressTensor,X,Y,Z,FaultFileString,varargin)
...
hold on
PostSlipStressDirections = figure('Name','PostSlipStressDirections','NumberTitle','off');
title({'\fontsize{14}Principal directions: Output Stress Directions','\fontsize{8}S1, red, S2, green, S3, blue, scaled by magnitude'})
...
if chk==1 
    hold on
    trisurf(Triangles,Points(:,2),Points(:,3),Points(:,4),'FaceAlpha',(.1),'FaceColor', [0.5 0 0.9 ]);
    hold on
end
hold on
...
filename = strcat(FaultFileString,'PostSlipStressDirections');
filename2 = strcat('OutputFigures/',filename);
saveas(PostSlipStressDirections,filename2, 'png')
