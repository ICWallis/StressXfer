function  DrawS1S2S3Directions(StressTensor,X,Y,Z,FaultFileString,varargin)
...
PrincDir = figure('Name','PrincipalDirections','NumberTitle','off');
...
assignin('base','S1Flag',S1Flag);
assignin('base','S2Flag',S2Flag);
assignin('base','S3Flag',S3Flag);
 assignin('base','S1x',S1x);
 assignin('base','S1y',S1y);
 assignin('base','S1z',S1z);
 assignin('base','S2x',S2x);
 assignin('base','S2y',S2y);
 assignin('base','S2z',S2z);
 assignin('base','S3x',S3x);
 assignin('base','S3y',S3y);
 assignin('base','S3z',S3z);
 assignin('base','Xcv',Xcv);
 assignin('base','Ycv',Ycv);
 assignin('base','Zcv',Zcv);
...
title({'\fontsize{14}Principal directions','\fontsize{8}S1 red, S2 green, S3 blue, scaled by magnitude'})
...
function [Pdir,Pdirneg,Ndir,Ndirneg]=QuiverLines(X,Y,Z,Snx,Sny,Snz,Flag,Scl,S)
    ...
    elseif sum(~Flag) == numel(Flag)
       ...
       assignin('base','Pdir',Pdir);
       assignin('base','Pdirneg',Pdirneg);
       assignin('base','Ndir',Ndir);
       assignin('base','Ndirneg',Ndirneg);
    end
    
end  %end internal func
view(-35,15)

filename = strcat(FaultFileString,'PrincipalDirections');
filename2 = strcat('OutputFigures/',filename);
saveas(PrincDir,filename2, 'png')
end
