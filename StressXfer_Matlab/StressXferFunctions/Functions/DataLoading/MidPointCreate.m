...
%     Faultsandnormals = figure; trisurf(Triangles,Points(:,2),Points(:,3),Points(:,4),'FaceAlpha',(.8),'facecolor', 'cyan');
%     axis equal;
%     hold on;
%     quiver3(MidPoint(:,1),MidPoint(:,2),MidPoint(:,3), ...
%          FaceNormalVector(:,1),FaceNormalVector(:,2),FaceNormalVector(:,3),0.5, 'color','r');
%     xlabel('x'); ylabel('y');
%     title('Input fault(s) showing normals') 
%     saveas(Faultsandnormals,'OutputFigures/Faultsandnormals.png')
%     hold off;
%     WhiteFigure
% end
