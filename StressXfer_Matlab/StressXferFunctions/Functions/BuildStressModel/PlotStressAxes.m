function [Sxx3D,Syy3D,Szz3D,Sxy3D,Sxz3D,Syz3D] = PlotStressAxes(Sv3D,Shmax3D,Shmin3D,DirS1,InputElevation,FaultFileString)
...
if Test==1
   for i = 1:(length(Sv3D))
   [Sxx3D(i),Syy3D(i),Szz3D(i),Sxy3D(i),Sxz3D(i),Syz3D(i)]= TensorsFromPrincipal3d_normal(Sv3D(i),Shmax3D(i),Shmin3D(i),DirS1);
   end
    disp('Normal Faulting Stress Regime')
else
if Test2==1 && Test ~= 1
   for i = 1:(length(Sv3D))
   [Sxx3D(i),Syy3D(i),Szz3D(i),Sxy3D(i),Sxz3D(i),Syz3D(i)]= TensorsFromPrincipal3d_strikeslip(Shmax3D(i),Sv3D(i),Shmin3D(i),DirS1);
   end
    disp('Strike Slip Faulting Stress Regime')
else
    for i = 1:(length(Sv3D))
   [Sxx3D(i),Syy3D(i),Szz3D(i),Sxy3D(i),Sxz3D(i),Syz3D(i)]= TensorsFromPrincipal3d_thrust(Shmax3D(i),Shmin3D(i),Sv3D(i),DirS1);
   end
